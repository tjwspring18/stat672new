...
particular distribution aids the estimation of coefficients. 
...
$(t)$ refers to a particular iteration, $\bm{J}$ is the Jacobian,
and $\bm{H}$ is the Hessian. The practicality of Newton-Raphson thus depends on whether it is convenient to
...
function for each observation. We ``know'' (assume) the form of this PDF and so are
